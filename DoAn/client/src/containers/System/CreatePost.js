...
import { getCodes, getCodesArea } from "../../untils/Common/getCodes";
import { useDispatch, useSelector } from "react-redux";
import { apiCreatePost } from "../../services";
import Swal from "sweetalert2";
import validate from "../../untils/Common/validateDields";
    ...
    if (result === 0) {
      if (dataEdit && isEdit) {
        ...
        if (response?.data.err === 0) {
          Swal.fire("Thanh cong", "Da sua thanh cong", "success").then(() => {
            resetPayload();
            dispatch(resetDataEdit());
          });
        } else {
          Swal.fire("Oops!", "Co loi gi do", "error");
        }
      } else {
        // console.log("finallllll", finalPayload);
        const response = await apiCreatePost(finalPayload);
        if (response?.data.err === 0) {
          Swal.fire("Thanh cong", "Da them bai dang moi", "success").then(
            () => {
              resetPayload();
            }
          );
